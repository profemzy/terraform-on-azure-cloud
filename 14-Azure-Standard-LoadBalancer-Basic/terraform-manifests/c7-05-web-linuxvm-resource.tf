...
#!/bin/bash
# Configure DNF to skip unavailable repositories (RHEL 10 Azure RHUI workaround)
echo "skip_if_unavailable=True" | sudo tee -a /etc/dnf/dnf.conf
# Install Apache with timeout and skip options
sudo dnf install -y httpd --setopt=skip_if_unavailable=True --setopt=timeout=10
